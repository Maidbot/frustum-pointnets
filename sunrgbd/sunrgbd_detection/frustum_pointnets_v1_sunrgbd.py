...
def placeholder_inputs(batch_size, num_point):
    ...
    one_hot_vec_pl = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, NUM_CLASS))
    labels_pl = tf.compat.v1.placeholder(tf.int32,
                                shape=(batch_size, num_point))
    centers_pl = tf.compat.v1.placeholder(tf.float32,
                                shape=(batch_size, 3))
...
def get_model(point_cloud, one_hot_vec, is_training, bn_decay=None):
    ...
    mask_count = tf.tile(tf.reduce_sum(input_tensor=mask,axis=1,keepdims=True), [1,1,3]) # Bx1x3
    print mask
    point_cloud_xyz = tf.slice(point_cloud, [0,0,0], [-1,-1,3]) # BxNx3

    # ---- Subtract points mean ----
    mask_xyz_mean = tf.reduce_sum(input_tensor=tf.tile(mask, [1,1,3])*point_cloud_xyz, axis=1, keepdims=True) # Bx1x3
...
def get_loss(logits, \
    ...
    mask_loss = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=mask_label))
    ...
    stage1_center_dist = tf.norm(tensor=center_label - end_points['stage1_center'], axis=-1)
    stage1_center_loss = huber_loss(stage1_center_dist, delta=1.0)
    tf.compat.v1.summary.scalar('stage1 center loss', stage1_center_loss)

    heading_class_loss = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=end_points['heading_scores'], labels=heading_class_label))
    ...
    heading_residual_normalized_loss = huber_loss(tf.reduce_sum(input_tensor=end_points['heading_residuals_normalized']*tf.cast(tmp, dtype=tf.float32), axis=1) - heading_residual_normalized_label, delta=1.0)
    print heading_residual_normalized_loss
    tf.compat.v1.summary.scalar('heading residual normalized loss', heading_residual_normalized_loss)

    size_class_loss = tf.reduce_mean(input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=end_points['size_scores'], labels=size_class_label))
    ...
    tmp2_tiled = tf.tile(tf.expand_dims(tf.cast(tmp2, dtype=tf.float32), -1), [1,1,3]) # BxNUM_SIZE_CLUSTERx3
    predicted_size_residual_normalized = tf.reduce_sum(input_tensor=end_points['size_residuals_normalized']*tmp2_tiled, axis=[1]) # Bx3

    tmp3 = tf.expand_dims(tf.constant(mean_size_arr, dtype=tf.float32),0) # 1xNUM_SIZE_CLUSTERx3
    mean_size_label = tf.reduce_sum(input_tensor=tmp2_tiled * tmp3, axis=[1]) # Bx3
    size_residual_label_normalized = size_residual_label / mean_size_label
 
    size_normalized_dist = tf.norm(tensor=size_residual_label_normalized - predicted_size_residual_normalized, axis=-1)
    size_residual_normalized_loss = huber_loss(size_normalized_dist, delta=1.0)
    print size_residual_normalized_loss
    tf.compat.v1.summary.scalar('size residual normalized loss', size_residual_normalized_loss)

    # Compute IOU 3D
    iou2ds, iou3ds = tf.compat.v1.py_func(compute_box3d_iou, [end_points['center'], end_points['heading_scores'], end_points['heading_residuals'], end_points['size_scores'], end_points['size_residuals'], center_label, heading_class_label, heading_residual_label, size_class_label, size_residual_label], [tf.float32, tf.float32])
    ...
    corners_3d_pred = tf.reduce_sum(input_tensor=tf.cast(tf.expand_dims(tf.expand_dims(gt_mask,-1),-1), dtype=tf.float32)*corners_3d, axis=[1,2]) # (B,8,3)
    ...
    heading_label = tf.reduce_sum(input_tensor=tf.cast(tmp, dtype=tf.float32)*heading_label, axis=1)
    ...
    size_label = tf.reduce_sum(input_tensor=tf.expand_dims(tf.cast(tmp2, dtype=tf.float32),-1)*size_label, axis=[1]) # (B,3)
    ...
    corners_dist = tf.minimum(tf.norm(tensor=corners_3d_pred - corners_3d_gt, axis=-1), tf.norm(tensor=corners_3d_pred - corners_3d_gt_flip, axis=-1))
